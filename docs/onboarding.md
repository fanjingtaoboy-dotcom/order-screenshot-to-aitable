...
这两个是必备的。其余 `dingtalk-*` 技能属于其他钉钉产品，装不装都不影响录单。
...
从返回中找到订单编号、下单时间、收货号码三个字段的 `fieldId`，填进 `target-table.json`。
...
看到 `ok: true` 才算配置完成。这一步会检查登录态、字段类型、目标表是否存在。
...
日常使用只需要一样：**订单截图**。某条订单需要备注时，把备注内容一起说明即可。
...
```text
订单 6929387397478383133 备注：客户要求改地址，发货前先确认。
```

多条订单时请带上订单编号，否则它会先问清楚是哪一单，不会随意套用。
...
**订单备注是可选的。** 只有你提到备注时才会写这一列；没提到的订单不会写入备注。多条订单同时存在时，备注必须说清对应哪一单。
...
| 提示备注没说清是哪一单 | 多条订单时备注必须带订单编号，按「订单编号=备注内容」说明 |
| 写入失败提示无权限 | 当前账号对该表只有查看权限，需要编辑权限 |
| 记录写进去了但视图里看不到 | 该视图可能带有筛选条件（例如按承接助教筛选），换到不带筛选的视图查看 |
...
订单备注是可选的。使用者描述某条订单需要备注时，这段内容会写进该订单的「订单备注」列。
...
| 多条订单，备注没写编号 | 停下追问，不会随意套用 |
| 订单编号对不上 | 停下，请核对编号 |
| 同一订单给了两条备注 | 停下，不猜用哪条 |
| 该订单已存在于表中 | 跳过整条，备注也不写入 |

## 十一、关于承接助教

这个 skill 只写订单编号、下单时间、虚拟手机号，以及可选的订单备注。

**承接助教不由它处理**，既不询问姓名，也不写入该字段。新记录的助教列会留空。

如果目标表里有按承接助教筛选的子视图（例如「录单表（某某）」），新写入的记录不会出现在这些视图里，需要先在表内指派助教。
