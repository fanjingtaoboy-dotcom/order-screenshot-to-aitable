...
你给它两张东西：一批订单截图，以及这批订单分配给哪位助教。它负责读取截图里的订单编号、下单时间、收货人虚拟号信息，校验去重之后追加到表格里，并在写入前后验证表格原有内容一条都没被改动。
...
1. **写入前预检**：核对表格字段类型、可写字段、助教身份是否有效。
...
- 承接助教必须由本次使用者明确指定。没给姓名会先追问，不先录入。
...
- Codex（具备读图能力）
- DWS CLI，并完成钉钉登录
- 钉钉技能包：`dingtalk-aitable`、`dingtalk-shared`、`dingtalk-aisearch`、`dingtalk-contact`
- 当前账号对目标多维表有**编辑**权限
- 承接助教与写入账号在**同一个组织**内
...
```bash
python3 scripts/preflight_check.py --assistant "<你的助教姓名>"
```
...
- **去重按订单编号进行**，所以同一个订单无法用它来改派助教。已存在的记录会被跳过，改派需要在表格里手工操作。
- **助教姓名由人提供**，系统只能保证这个名字在当前组织里真实且唯一，无法判断人有没有说错。
