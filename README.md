...
你给它一批订单截图，它负责读取截图里的订单编号、下单时间、收货人虚拟号信息，校验去重之后追加到表格里，并在写入前后验证表格原有内容一条都没被改动。如果某条订单需要备注，把备注内容一起告诉它，会写进该订单的「订单备注」列。
...
订单备注是可选信息，不影响主流程。没有备注的订单不会写入备注列。
...
- **备注必须能对应到具体订单**。多条订单时如果备注没说清是哪一单，会停下追问，不会随意套用。
- **已存在的订单不会补写备注**。重复订单会被整条跳过，备注也不写入，避免覆盖表里已有内容。
